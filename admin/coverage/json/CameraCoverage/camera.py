...
class Camera:
    ...
    def FOVstatic(self,maxLength=-1):
        min_ccd_h = -self.ccd_h / 2.0
        if maxLength > 0:
            min_ccd_h_tmp = - tan(self.tilt - atan2(self.location[2],maxLength)) * self.f
            if min_ccd_h_tmp > min_ccd_h:
                min_ccd_h = min_ccd_h_tmp
        rays = np.array([[-self.ccd_w/2.0 , self.ccd_h/2.0 , self.f],
                [self.ccd_w/2.0 , self.ccd_h/2.0 , self.f],
                [self.ccd_w/2.0 , min_ccd_h , self.f],
                [-self.ccd_w/2.0 , min_ccd_h , self.f]])
...
if __name__ == "__main__":
    ...
    ccd_w = 3264.0
    ccd_h = 2448.0
    c = Camera(680250.528, 3555005.36, 13.85, (78.852587)/180.0 * pi ,32.9/180.0 * pi, ccd_w, ccd_h, 2056.063)
